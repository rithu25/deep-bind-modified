...
file_list = glob.glob(os.path.join(os.getcwd(), "/Users/rithikavenkat/Desktop/", "*.fa"))
...
for i in new_sequences:
    if len(i) > 20:
        final_seq.append([i[j:j+n] for j in range(0, len(i), 3)])
    else:
        final_seq.append([i])
print (final_seq)
...
#        for j in range(len(final_seq[i])):
#            print(final_seq[i][j])
    ...
    for i,a in zip(final_seq,dna):
        ...
        for k in range(len(i)):
            ...
            #print(l)
            print("\n".join(str(x) for x
                            in model.predict(np.array(onehot_sequences[k:k+1])[:,:,:]))) 
            l = l+3
